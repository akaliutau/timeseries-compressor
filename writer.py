...
from bitbuffer import BitBufferWriter, BufferWriter
...
class BlockWriter:
    ...
    def t_int16(self, value: int) -> None:
        if value == 0:
            self.buf.add_value(0, 1)  # bits 0
        elif t_uint8(value) < 256:
            self.buf.add_value(2, 2)
            self.buf.add_value(t_uint8(value), 8)  # bits 10|8bit
        else:
            self.buf.add_value(3, 2)
            self.buf.add_value(t_uint16(value), 16)  # bits 11|16bit

    def t_int32(self, value: int) -> None:
        if value == 0:
            self.buf.add_value(0, 1)  # bits 0
        elif t_uint8(value) < 256:
            self.buf.add_value(2, 2)
            ...
            self.buf.add_value(3, 2)
            self.buf.add_value(t_uint32(value), 32)  # bits 11|32bit

    def t_int64(self, value: int) -> None:
        if value == 0:
            self.buf.add_value(0, 1)  # bits 0
        elif t_uint8(value) < 256:
            self.buf.add_value(2, 2)
            ...
            self.buf.add_value(3, 2)
            self.buf.add_value(t_uint64(value), 64)  # bits 11|64bit

    def t_float64(self, value: int) -> None:
        if value == 0:
            self.buf.add_value(0, 1)  # bits 0
            return
        ...
        self.buf.add_value(1, 1)  # bits 1
        self.buf.add_value(first_non_zero_byte, 3)  # bits 3
        self.buf.add_value(important_bytes, 3)  # bits 3
        shift_in_bits = (7 - last_non_zero_byte) * 8
        value = value >> shift_in_bits
        self.buf.add_value(value, (important_bytes + 1) * 8)  # bits 1  # bits 1|3bit|3bit|data

    def t_str(self, value: int) -> None:
        if value == 0:
            self.buf.add_value(0, 1)  # bits 0
            return
        self.buf.add_value(1, 1)  # bits 1
        self.buf.add_value(value, 16)  # bits 16 for index TODO can be optimized

    def t_array(self, arr: List[int]) -> None:
        if not arr:
            self.buf.add_value(0, 1)  # bits 0
            return
        ...
        important_bytes = last_non_zero_byte - first_non_zero_byte
        self.buf.add_value(1, 1)  # bits 1
        self.buf.add_value(first_non_zero_byte, 5)  # bits 5
        self.buf.add_value(important_bytes, 5)  # bits 5
        i = first_non_zero_byte
        while i <= last_non_zero_byte:
            self.buf.add_value(arr[i], 8)  # bits 1  # bits 1|5bit|5bit|data
            i += 1
